...
public class BlackJack {
	...
	public void setPlayerHands(GuildMessageReceivedEvent event) {
		...
		eb2.setTitle("Black Jack: Your Hand");
		eb2.setDescription("Total: " + getPlayerNum());
		eb2.setThumbnail("attachment://" + filesInDir[num2].getName());
		eb2.setColor(Color.green);
		event.getChannel().sendMessage(eb2.build()).addFile(card2, filesInDir[num2].getName()).queue();
	...
	public void setBotHands(GuildMessageReceivedEvent event) {
		...
		botCards.add(num2);
		File card1 = new File("./cards/" + filesInDir[num].getName());
		File card2 = new File("./cards/" + filesInDir[0].getName());

		EmbedBuilder eb = new EmbedBuilder();
		eb.setTitle("Black Jack: Bot Hand");
		eb.setThumbnail("attachment://" + filesInDir[num].getName());
		eb.setColor(Color.red);
		event.getChannel().sendMessage(eb.build()).addFile(card1, filesInDir[num].getName()).queue();

		EmbedBuilder eb2 = new EmbedBuilder();
		eb2.setTitle("Black Jack: Bot Hand");
		eb2.setThumbnail("attachment://" + filesInDir[0].getName());
		eb2.setColor(Color.red);
		//event.getChannel().sendMessage(eb2.build()).addFile(card2, filesInDir[0].getName()).queue();
	...
	public void botHit(GuildMessageReceivedEvent event) {
		...
		EmbedBuilder eb = new EmbedBuilder();
		eb.setTitle("Black Jack: Bot Hand");
		eb.setDescription("Total: " + getBotNum());
		eb.setThumbnail("attachment://" + filesInDir[num].getName());
		eb.setColor(Color.red);
		//event.getChannel().sendMessage(eb.build()).addFile(card1, filesInDir[num].getName()).queue();
	}
